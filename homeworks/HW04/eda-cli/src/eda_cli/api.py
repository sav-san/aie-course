...
class FlagResponse(BaseModel):
    flags: dict[str, object] | None = Field(
        default=None,
        description="Все флаги качества",
    )
...
async def quality_from_csv(file: UploadFile = File(...)) -> QualityResponse:
    ...
    return QualityResponse(
        ...
        latency_ms=latency_ms,
        flags=flags_bool,
        ataset_shape={"n_rows": n_rows, "n_cols": n_cols},
    )

@app.post("/quality-flags-from-csv",summary="Возвращает полный набор флагов качества с использованием EDA-ядра",tags=["quality"],response_model=FlagResponse)
